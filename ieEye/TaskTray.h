...
class CTaskTray : public CWindowImpl<CTaskTray>,
  public CTrayIconImpl<CTaskTray>, public CUpdateUI<CTaskTray>
{
public:
  DECLARE_WND_CLASS(_T("TrayIcon"));

  BEGIN_UPDATE_UI_MAP(CTaskTray)
    UPDATE_ELEMENT(IDM_CHECKENABLE, UPDUI_MENUPOPUP)
  END_UPDATE_UI_MAP()
  ...
  BEGIN_MSG_MAP(CTaskTray)
    ...
    COMMAND_ID_HANDLER_EX(IDM_CHECKENABLE, OnToggleCheck)
    COMMAND_ID_HANDLER_EX(ID_MENU1_EXIT, OnMenu1Exit)
    CHAIN_MSG_MAP(CUpdateUI<CTaskTray>)
  END_MSG_MAP()
  ...
  int OnCreate(LPCREATESTRUCT lpCreateStruct){
    ...
    bTimerEnabled = TRUE;
    UISetCheck(IDM_CHECKENABLE, bTimerEnabled);
    return 0;
  }
  ...
  void OnToggleCheck(UINT uNotifyCode, int nID, CWindow wndCtl)
  {
    if(bTimerEnabled)KillTimer(ID_CHECKTIMER);
    else SetTimer(ID_CHECKTIMER, 1000*10, NULL);
